...
export default function ResultsAnalytics({ pendingResults, classPerformance, subjectPerformance, schoolAveragePercent, onPublish }: ResultsAnalyticsProps) {
  ...
  return (
    <div className="pb-10">
      <h1 className="font-display text-[18px] font-semibold text-ink dark:text-white sm:text-[20px]">Results &amp; analytics</h1>
      <p className="mt-0.5 text-[13px] text-ink/50 dark:text-white/50">School-wide publishing and performance, across every class and subject.</p>
      ...
      {/* ---------- Pending publication ---------- */}
      <div className="mt-6">
        <h2 className="mb-3 font-display text-[15px] font-semibold text-ink dark:text-white">Pending publication</h2>
        {pendingResults.length === 0 ? (
          <div className="flex items-center gap-2.5 rounded-lg border border-dashed border-black/10 dark:border-white/15 bg-white dark:bg-[#1A1C20] px-4 py-6 text-[13px] text-ink/45 dark:text-white/45">
            <CheckCircle2 size={16} className="text-ink/25 dark:text-white/25" />
            ...
            {pendingResults.map((r) => (
              <div key={r.examId} className="flex items-center gap-3.5 px-4 py-3.5 sm:px-5">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-[12px] text-ink/50 dark:text-white/50">{r.subjectName} · {r.className} · {r.teacherName}</p>
                </div>
                <div className="shrink-0 text-right">
                  <p className="text-[12.5px] font-semibold tabular-nums text-ink/70 dark:text-white/70">{r.averageScore.toFixed(1)}/{r.maxScore} avg</p>
                  <p className="text-[11px] text-ink/40 dark:text-white/40">{r.studentCount} students</p>
                </div>
                <button
      ...
      <div className="mt-6">
        <h2 className="mb-3 font-display text-[15px] font-semibold text-ink dark:text-white">Performance by class</h2>
        <div className="rounded-lg border border-black/5 dark:border-white/10 bg-white dark:bg-[#1A1C20] p-4.5 sm:p-5">
      ...
      <div className="mt-6">
        <h2 className="mb-3 font-display text-[15px] font-semibold text-ink dark:text-white">Performance by subject</h2>
        <div className="rounded-lg border border-black/5 dark:border-white/10 bg-white dark:bg-[#1A1C20] p-4.5 sm:p-5">
          <div className="space-y-2.5">
      ...
      {confirmExamId && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-ink/50 p-4">
          <div className="w-full max-w-sm rounded-lg bg-white dark:bg-[#1A1C20] p-6 shadow-card-hover">
            <h2 className="font-display text-[15px] font-semibold text-ink dark:text-white">Publish this result?</h2>
            <p className="mt-1.5 text-[13px] text-ink/60 dark:text-white/60">Students in this class will be able to see their score and grade immediately.</p>
            {publishError && <p className="mt-3 rounded-md bg-crimson-50 dark:bg-crimson-600/15 px-3 py-2 text-[12.5px] text-crimson-700 dark:text-crimson-500">{publishError}</p>}
            <div className="mt-4 flex gap-2.5">
              <button onClick={() => { setConfirmExamId(null); setPublishError(null); }} className="flex-1 rounded-lg border border-black/10 dark:border-white/15 py-2.5 text-[13px] font-medium text-ink/70 dark:text-white/70 hover:bg-background-muted dark:hover:bg-white/5">
                Cancel
              </button>
              <button
...
function StatCard({ icon: Icon, label, value, tone = "default" }: { icon: React.ElementType; label: string; value: string; tone?: "success" | "warn" | "default" }) {
  ...
  return (
    <div className="rounded-lg border border-black/5 dark:border-white/10 bg-white dark:bg-[#1A1C20] p-4 shadow-card">
      <div className="mb-2 flex h-8 w-8 items-center justify-center rounded-md bg-crimson-50 dark:bg-crimson-600/15 text-crimson-700 dark:text-crimson-500">
        <Icon size={16} />
      </div>
...
function BarRow({ label, percent }: { label: string; percent: number }) {
  return (
    <div className="flex items-center gap-3">
      <span className="w-28 shrink-0 truncate text-[12.5px] text-ink/70 dark:text-white/70">{label}</span>
      <div className="h-3 flex-1 overflow-hidden rounded-full bg-background-muted dark:bg-white/5">
        <div className={`h-full rounded-full ${percent < 40 ? "bg-crimson-600" : percent < 70 ? "bg-warning" : "bg-success"}`} style={{ width: `${percent}%` }} />
      </div>
      <span className="w-10 shrink-0 text-right text-[12px] font-medium text-ink/60 dark:text-white/60">{percent.toFixed(0)}%</span>
    </div>
  );
}
