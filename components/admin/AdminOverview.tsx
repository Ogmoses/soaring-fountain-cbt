...
export default function AdminOverview({ schoolName, stats, todayBatches, conflictBatchIds }: AdminOverviewProps) {
  return (
    <div className="pb-10">
      <h1 className="font-display text-[18px] font-semibold text-ink dark:text-white sm:text-[20px]">{schoolName} — Overview</h1>
      ...
      {conflictBatchIds.length > 0 && (
        <div className="mt-5 flex items-start gap-2.5 rounded-lg border border-crimson-200 bg-crimson-50 px-4 py-3 text-[13px] text-crimson-700 dark:border-crimson-600/30 dark:bg-crimson-600/10 dark:text-crimson-500">
      ...
      <div className="mt-6">
        <h2 className="mb-3 font-display text-[15px] font-semibold text-ink dark:text-white">Today's lab batches</h2>
          ...
          <div className="divide-y divide-black/5 rounded-lg border border-black/5 bg-white dark:divide-white/10 dark:border-white/10 dark:bg-[#1A1C20]">
            {todayBatches
              ...
              .map((b) => {
                ...
                return (
                  <div key={b.id} className={`flex items-center gap-3.5 px-4 py-3.5 sm:px-5 ${conflict ? "bg-crimson-50/40 dark:bg-crimson-600/10" : ""}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-[12px] text-ink/50 dark:text-white/50">{b.subjectName} · {b.className} · {b.batchLabel}</p>
                    </div>
                    <div className="shrink-0 text-right">
                      <p className="text-[12px] font-medium text-ink/70 dark:text-white/70">{b.labRoom ?? "Main lab"}</p>
                      <p className="text-[11px] text-ink/40 dark:text-white/40">{b.studentCount} students</p>
                    </div>
...
function StatCard({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: number }) {
  return (
    <div className="rounded-lg border border-black/5 bg-white p-4 shadow-card dark:border-white/10 dark:bg-[#1A1C20]">
      <div className="mb-2 flex h-8 w-8 items-center justify-center rounded-md bg-crimson-50 text-crimson-700 dark:bg-crimson-600/15 dark:text-crimson-500">
        <Icon size={16} />
      </div>
