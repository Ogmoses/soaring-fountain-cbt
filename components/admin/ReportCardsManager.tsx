...
export default function ReportCardsManager({
  ...
  return (
    <div className="pb-10">
      <h1 className="font-display text-[18px] font-semibold text-ink dark:text-white sm:text-[20px]">Report cards</h1>
      <p className="mt-0.5 text-[13px] text-ink/50 dark:text-white/50">Terminal report cards, generated straight from published results.</p>

      <div className="mt-4 flex flex-col gap-2.5 sm:flex-row">
        <select value={selectedClassId} onChange={(e) => onClassChange(e.target.value)} className="rounded-lg border border-black/10 dark:border-white/15 px-3.5 py-2.5 text-[13px] outline-none focus:border-crimson-500">
          {classOptions.map((c) => (
            <option key={c.id} value={c.id}>{c.name}</option>
          ))}
        </select>
        <select value={selectedTermId} onChange={(e) => onTermChange(e.target.value)} className="rounded-lg border border-black/10 dark:border-white/15 px-3.5 py-2.5 text-[13px] outline-none focus:border-crimson-500">
          {termOptions.map((t) => (
      ...
      {roster.length === 0 ? (
        <div className="mt-5 flex items-center gap-2.5 rounded-lg border border-dashed border-black/10 dark:border-white/15 bg-white dark:bg-[#1A1C20] px-4 py-8 text-[13px] text-ink/45 dark:text-white/45">
          <GraduationCap size={16} className="text-ink/25 dark:text-white/25" />
          ...
          {roster.map((s) => (
            <div key={s.studentId} className="flex flex-col gap-2.5 px-4 py-3.5 sm:flex-row sm:items-center sm:px-5">
              <div className="min-w-0 sm:w-48 sm:shrink-0">
                <p className="truncate text-[13.5px] font-medium text-ink dark:text-white">{s.fullName}</p>
                <p className="text-[12px] text-ink/45 dark:text-white/45">{s.admissionNumber}</p>
              </div>
              <input
                ...
                className="flex-1 rounded-lg border border-black/10 dark:border-white/15 px-3 py-2 text-[12.5px] outline-none focus:border-crimson-500"
              />
              <div className="flex shrink-0 gap-1.5">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-1.5 rounded-lg border border-black/10 dark:border-white/15 px-3 py-2 text-[12.5px] font-medium text-ink/70 dark:text-white/70 hover:bg-background-muted dark:hover:bg-white/5"
                >
                  <Eye size={14} /> Preview
                </a>
                <a
