  ...
  return (
    <div className="pb-10">
      <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          <h1 className="font-display text-[18px] font-semibold text-ink dark:text-white sm:text-[20px]">Class analytics</h1>
          {data && <p className="mt-0.5 text-[13px] text-ink/50 dark:text-white/50">{data.className} · {data.subjectName}</p>}
        </div>
        <select
              ...
              {data.scoreDistribution.map((band) => {
                ...
                return (
                  <div key={band.label} className="flex items-center gap-3">
                    <span className="w-16 shrink-0 text-[12px] text-ink/50 dark:text-white/50">{band.label}</span>
                    ...
                    <span className="w-6 shrink-0 text-right text-[12px] font-medium text-ink/60 dark:text-white/60">{band.count}</span>
                  </div>
                );
              })}
            </div>
              ...
              {[...data.questionDifficulty]
                ...
                .map((q, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className="flex-1 truncate text-[12.5px] text-ink/75 dark:text-white/75">{q.prompt}</span>
                    <div className="h-2 w-24 shrink-0 overflow-hidden rounded-full bg-background-muted dark:bg-white/5">
                      <div
                    ...
                    <span className="w-10 shrink-0 text-right text-[12px] font-medium text-ink/60 dark:text-white/60">{q.correctPercent.toFixed(0)}%</span>
                  </div>
                ))}
            </div>
          </SectionCard>
          ...
          <SectionCard title="Student results" className="mt-5">
              ...
              {data.students.map((s) => (
                <div key={s.id} className="flex items-center justify-between gap-3 py-2.5">
                  <div className="flex items-center gap-2.5">
                    <span className="w-6 shrink-0 text-[12px] font-semibold text-ink/40 dark:text-white/40">#{s.rank}</span>
                    <span className="text-[13px] font-medium text-ink dark:text-white">{s.name}</span>
                  </div>
            ...
            <table className="hidden w-full text-left sm:table">
              ...
              <tbody className="divide-y divide-black/5 dark:divide-white/10">
                {data.students.map((s) => (
                  <tr key={s.id} className="text-[13px] text-ink dark:text-white">
                    <td className="py-2.5 pr-4 text-ink/50 dark:text-white/50">#{s.rank}</td>
                    <td className="py-2.5 pr-4 font-medium">{s.name}</td>
                    <td className="py-2.5 pr-4 tabular-nums">{s.score}/{s.maxScore}</td>
                    <td className="py-2.5 text-ink/70 dark:text-white/70">{s.grade ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function StatCard({ label, value, tone = "default" }: { label: string; value: string; tone?: "success" | "warn" | "default" }) {
  ...
  return (
    <div className="rounded-lg border border-black/5 dark:border-white/10 bg-white dark:bg-[#1A1C20] p-4 shadow-card">
      ...
      <p className="mt-0.5 text-[11.5px] text-ink/45 dark:text-white/45">{label}</p>
    </div>
  );
}
